...
// end setup for MCP4725 DAC


//#define N2K_Output true  // set to true to get Serial output for debugging
#define N2K_Output false  // set to true to get Serial output for debugging
...
//bool SerialPlotMode = true;
bool SerialPlotMode = false;
...
//*************************part of multi-device start *******************************

const unsigned long CompassTransmitMessages[] PROGMEM = {127250L, 127251L, 127257, 0};


//sets the frequency of different messages on the N2K bus

#define headingUpdatePeriod 100

#define DEV_COMP 0


//*************************part of multi-device end*******************************
...
//  Wire.setClock(400000); // I2C frequency at 400 kHz
  ...
  serial_input=49;
    ...
    // Take user input to choose load accelerometer callibration or not...
    // "2" from the keboard is ASCII "2" which gives integer value 50
    // "0" from the keboard is ASCII "0" which gives integer value 48
  ...
  if (!passThru)
  {
    ...
    Serial.print("Magnetometer Default scale Scale Range: +/-"); Serial.print(EM7180_mag_fs); Serial.println("uT");
    ...
    EM7180_set_mag_acc_FS (0x3E8, 0x08); // 1000 uT, 8 g  //JS original values
    EM7180_set_gyro_FS (0x7D0); // 2000 dps  //JS original values
//      EM7180_set_mag_acc_FS (0x3E8, 0x02); // 1000 uT, 2 g
//      EM7180_set_gyro_FS (0xFA); // 250 dps
  ...
  NMEA2000.SetDeviceCount(1); // Enable multi device support for 1 devices


  // Set Product information for Digital Compass
  NMEA2000.SetProductInformation("2049", // Manufacturer's Model serial code.
                                 102, // Manufacturer's product code
                                 "Digital Compass",  // Manufacturer's Model ID
                                 "1.0.0.6 (2017-06-13)",  // Manufacturer's Software version code
                                 "1.0.4.0 (2017-06-13)", // Manufacturer's Model version
  ...
  // Set device information for Digital Compass
  NMEA2000.SetDeviceInformation(0123333,  // Unique number. Use e.g. Serial number.
                                160,    // Device function=Rudder. See codes on http://www.nmea.org/Assets/20120726%20nmea%202000%20class%20&%20function%20codes%20v%202.00.pdf
                                40,     // Device class=Electrical Generation. See codes on  http://www.nmea.org/Assets/20120726%20nmea%202000%20class%20&%20function%20codes%20v%202.00.pdf
                                2042,    // Just choosen free from code list on http://www.nmea.org/Assets/20121020%20nmea%202000%20registration%20list.pdf
                                4, // Marine
                                DEV_COMP
                               );
  ...
  // Here we tell, which PGNs we transmit from compass  module
  NMEA2000.ExtendTransmitMessages(CompassTransmitMessages, DEV_COMP);

  NMEA2000.Open();


analogWriteResolution(12); //sets teensy DAC resolution

dac.begin(0x63); //  stand alone DAC  MCP4725A1 is on 0x63 
...
dac.setVoltage (1234,false); //testing the MCP4725 DAC output
...
//  MadgwickQuaternionUpdate(ax, ay, az, gx * PI / 180.0f, gy * PI / 180.0f, gz * PI / 180.0f,  mx,  my, mz);
  ...
  if (delt_t > 200)
  {
      ...
      analogWriteDAC0((int)Yaw*10);
      
      analogWriteDAC1((int)Roll*10);
      
      //analogWrite(A14, (int)Yaw*10);
...
//    roll  *= 180.0f / PI;
    ...
    Yaw   += Declination; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
    Yaw   += CompassOffset; //offset of the compass from the keel line of the vessel
    if (Yaw < 0) Yaw   += 360.0f ; // Ensure yaw stays between 0 and 360
    Roll  *= 180.0f / PI;
...
//  Wire.endTransmission();               // for use with Butterfly board wire.h instead the line above
